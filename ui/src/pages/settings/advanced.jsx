...
class AdancedSettingsPage extends Component {
  disconnectHomekit = () => {
    confirm({
      ...
      onOk: () => {
        ...
        client.on('connect', () => client.publish('hiome/1/api', 'homekitReset'))
      },
      okText: 'Reset',
      okType: 'danger'
    })
  ...
  disconnectHue = () => {
    confirm({
      ...
      onOk: () => {
        const client = connect(`ws://${window.location.host}:1884`)
        client.on('connect', () => client.publish('hiome/1/api', 'hueReset'))
      },
      okText: 'Reset',
      okType: 'danger'
    })
  ...
  factoryReset = () => {
    confirm({
      ...
      onOk: () => {
        const client = connect(`ws://${window.location.host}:1884`)
        client.on('connect', () => client.publish('hiome/1/api', 'factoryReset'))
      },
      okText: 'Reset',
      okType: 'danger'
    })
  ...
  updateSensors = () => {
    confirm({
      ...
      onOk: () => {
        const client = connect(`ws://${window.location.host}:1884`)
        client.on('connect', () => client.publish('hiome/1/api', 'updateFirmware'))
      },
      okText: 'Update',
      okType: 'primary'
    })
